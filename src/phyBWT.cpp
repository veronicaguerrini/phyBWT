...
int main(int argc, char **argv) {
	...
	std::cerr << "USE_LIST " << USE_LIST <<  std::endl;
	...
	if( argc < 4) {
		std::cerr << "Error usage " << argv[0] << " fileFasta fileInfo k_min tau t"  << std::endl; 
		exit(1);
	}
	...
	sscanf(argv[3], "%hhu", &k_min);
	sscanf(argv[4], "%lf", &tau);
	sscanf(argv[5], "%u", &max_it);
	
	cout << "k_min: " << (int)k_min << endl;
	cout << "tau: " << tau << endl;
	cout << "t: " << max_it << endl;

	cerr << "sizeof(dataTypeNChar)=" << sizeof(dataTypeNChar) << " bytes."<< endl;
	cerr << "sizeof(dataTypelenSeq)=" << sizeof(dataTypelenSeq) << " bytes."<< endl;
	cerr << "sizeof(dataTypeNSeq)=" << sizeof(dataTypeNSeq) << " bytes."<< endl;
    ...
    if (InEBWT==NULL){
        std::cerr << "Error opening " << fnEBWT << "." << std::endl;
        printf("fopen failed, errno = %d\n", errno);
        exit (EXIT_FAILURE);
	}
	...
	while(job_list.size()>0){
		...
		{ 
			...
			if(P.size()>2){
                ...
                #endif
				fprintf(stdout,"CALL Partition step: %d -> outputPartition.size=%lu\n",step,outputPartition.size());
			}
